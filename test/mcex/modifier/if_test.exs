defmodule Mcex.Modifier.IfTest do
  ...
  describe "modify/3" do
    ...
    test "works", %{mappings: mappings} do
      assert If.modify("foo", "r:fo t:b true dat f:b nah!", mappings) == {:ok, "true dat"}
      assert If.modify("more stuff", "t:b yep  r:ff$  f:b nah", mappings) == {:ok, "yep"}
      assert If.modify("howdy", "r:h.wdi  f:range 3  t:date", mappings) == {:ok, "1\n2\n3"}
    end
    ...
    test "works with ok tuples", %{mappings: mappings} do
      assert If.modify({:ok, "aaa"}, "r:.aa  t:b t  f:b f", mappings) == {:ok, "t"}
      assert If.modify({:ok, "aaa"}, "r:ba.  t:b t  f:b f", mappings) == {:ok, "f"}
    end

defmodule Mcex.HaveTest do
  ...
  describe "stats/2" do
    ...
    test "works with an empty date string" do
      assert Have.stats("", d(5)) == %{one: "n/a", tot: 0, hav: 0, avg: "infinity", cur: "n/a"}
      assert Have.stats("\n \t ", d(8)) == %{one: "n/a", tot: 0, hav: 0, avg: "infinity", cur: "n/a"}
    end

    test "works with 1 date" do
      assert Have.stats(ds(2), d(3)) == %{one: d(2), tot: 2, hav: 1, avg: 1, cur: 1}
      assert Have.stats(ds(11), d(15)) == %{one: d(11), tot: 5, hav: 1, avg: 4, cur: 4}
    end

    test "works with 1 date (where <today> == <the have date>)" do
      assert Have.stats(ds(2), d(2)) == %{one: d(2), tot: 1, hav: 1, avg: "n/a", cur: 0}
    end

    test "works with 2 or more dates (white space separated)" do
      assert Have.stats("#{ds(1)} #{ds(2)}", d(5)) == %{one: d(1), tot: 5, hav: 2, avg: 1.5, cur: 3}
    end

    test "works with 2 or more dates (where <today> == <last have date>)" do
      assert Have.stats(ds([1, 2]), d(2)) == %{one: d(1), tot: 2, hav: 2, avg: 0, cur: 0}
    end

    test "works with 2 or more dates (regardless of chronological order)" do
      assert Have.stats(ds([2, 1]), d(5)) == %{one: d(1), tot: 5, hav: 2, avg: 1.5, cur: 3}
      assert Have.stats(ds([2, 1, 5]), d(7)) == %{one: d(1), tot: 7, hav: 3, avg: 1.33, cur: 2}
      assert Have.stats(ds([9, 7, 1]), d(12)) == %{one: d(1), tot: 12, hav: 3, avg: 3.0, cur: 3}
    end
    ...
    test "treats duplicate dates as one date" do
      ...
      assert s1 == %{one: d(11), tot: 5, hav: 1, avg: 4, cur: 4}
      assert s1 == s2
    end

    test "errors with bad dates" do
      assert Have.stats("2014-01-05 2011-5-123", d(15)) == :error
